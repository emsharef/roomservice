...
import type { ArtistListItem, Pagination } from "@/lib/arternal";
    ...
    <div>
      ...
      {pagination && (
        <p className="mb-3 text-sm text-gray-500">
          {parseInt(pagination.total).toLocaleString()} artists total
            ...
            {artists.map((artist) => (
              ...
              >
                <td className="px-4 py-3">
                  <div className="text-sm font-medium text-gray-900">{artist.display_name}</div>
                  <div className="text-xs text-gray-500 sm:hidden">
                    {[artist.country, artist.work_count != null ? `${artist.work_count} works` : null].filter(Boolean).join(" · ") || null}
                  </div>
                </td>
                <td className="px-4 py-3 text-sm text-gray-600 hidden sm:table-cell">
                  {artist.country || <span className="text-gray-400">-</span>}
                </td>
                <td className="px-4 py-3 text-sm text-gray-900 text-right font-medium hidden sm:table-cell">
                  {artist.work_count ?? <span className="text-gray-400">-</span>}
                </td>
                <td className="px-4 py-3 text-sm text-gray-600 hidden lg:table-cell max-w-xs">
                  {truncate(artist.bio, 80) || <span className="text-gray-400">-</span>}
                </td>
                <td className="px-4 py-3 text-sm text-gray-600 hidden md:table-cell">
                  {artist.life_dates || <span className="text-gray-400">-</span>}
                </td>
              </tr>
            ))}
          </tbody>
